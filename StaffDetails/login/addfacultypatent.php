<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $issauth = mysqli_real_escape_string($db,$_POST['issauth']); 
	  $title = mysqli_real_escape_string($db,$_POST['title']); 
	  $patentno = mysqli_real_escape_string($db,$_POST['patentno']); 
      $validperiod = mysqli_real_escape_string($db,$_POST['validperiod']); 
	 
	
	   
      $sql = "insert into patenttl( title, issauth, patentno, validperiod, userid)values('$title', '$issauth', '$patentno', '$validperiod',$user_check)";
      ...
      if($result == TRUE) {
         ...
         header("location: facultypatents.php?msg=successfully added");
      }else {
         $error = "Not inserted";
		  echo $error;
      }
...
<div style = "font-size:11px; color:#cc0000; margin-top:10px"></div>
				  ...
				  <input required type="text" class="form-control" placeholder="Title of Patent" name="title">
				    <input required type="text" class="form-control" placeholder="Issuing Authority" name="issauth">
				 
			
			   <input required type="text" class="form-control" placeholder="Patent No" name="patentno">
			   <input required type="text" class="form-control" placeholder="Valid Period" name="validperiod">
