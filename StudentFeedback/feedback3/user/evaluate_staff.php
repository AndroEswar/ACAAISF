...
<?php 
$id = $_GET['id'];
$sql = mysql_query("select * from staff where staff_id='$id' ");?>
...
<div class="table-responsive">
	<form method="post">
		<table>
			...
			<p><?php echo $row['staff_firstname']." ".$row['staff_lastname']?></p>
			<input type="hidden" value="<?php $row['staff_id']; ?>" name="staff">
			</td>
...
$select = mysql_query("select * from staff where staff_id='$id'");
$row = mysql_fetch_array($select);
$idnum = $row['idnumber'];

 mysql_query("INSERT INTO `feedback`(`comments`, `category`, `idnumber`, `account_id`) 
 VALUES ('$comment','staff','$idnum','$session_id')");?>
 <script>
